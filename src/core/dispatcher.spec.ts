...
type MyListener<T = unknown> = Record<string, unknown> & { process?(...args: T[]): void };
...
describe('Dispatcher', () => {
  ...
  describe('listeners', () => {
    it('should not care about directly pushed listeners', () => {
      (dispatcher.listeners as unknown[]).push({});
      expect(dispatcher.listeners.length).toBe(0);
    });
  });
  ...
  describe('dispatch', () => {
    it('should call the given function name on each listener', () => {
      let calls = 0;
      const listeners = 10;
      for (let i = 0; i < listeners; i++) dispatcher.addListener({ process: () => calls++ });
    ...
    it('should not call the given function name on listeners which have been removed', () => {
      ...
      const toRemove = 5;
      for (let i = 0; i < listeners; i++) dispatcher.addListener({ process: () => calls++ });
      for (let i = 0; i < toRemove; i++) dispatcher.removeListener(listeners - 1 - i);
      dispatcher.dispatch('process');
    ...
    it('should only call the given function name on listeners which implemented the function', () => {
      ...
      for (let i = 0; i < listeners; i++) dispatcher.addListener({ process: () => calls++ });
      for (let i = 0; i < wrongListeners; i++) dispatcher.addListener({ processOther: () => calls++ });
      dispatcher.dispatch('process');
      expect(calls).toBe(listeners);
    });
  });
